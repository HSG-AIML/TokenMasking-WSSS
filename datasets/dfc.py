...
from datasets.dfc_sen12ms_dataset import (
    DFCSEN12MSDataset,
    LCBands,
    S1Bands,
    S2Bands,
...
from datasets.utils import AlbumentationsToTorchTransform
...
class DFCDataset(Dataset):
    ...
    def __init__(
        ...
        if mult > 1:
            self.observations = pd.DataFrame(
                np.repeat(self.observations.values, mult, axis=0),
                columns=self.observations.columns,
            )
        ...
        if self.transforms:
            augmentation = A.Compose(
                [
                    ...
                    # A.Resize(height=64, width=64, p=1, interpolation=cv2.INTER_NEAREST),
    ...
    def __getitem__(self, idx, s2_bands=S2Bands.ALL, transform=True, normalize=True):
        ...
        if self.mode != "sen12ms":
            ...
            dfc_multilabel = torch.tensor(
                [
                    ...
                    if num / self.image_px_size**2 >= self.frac
                    and class_idx != 255  # 0.1
                ]
            ).long()
        ...
        # dfc_multilabel_one_hot_fil = torch.Tensor([dfc_multilabel_one_hot[i] for i in [0, 2, 3, 4, 7]])
        if dfc is not None:
            output.update(
                {
                    "mask": dfc,
...
class DFCDataModule(pl.LightningDataModule):
    def __init__(
        self,
    ):
        super(DFCDataModule).__init__()

    def prepare_data(self):
        pass

    def setup(self, mult, train_batch_size, eval_batch_size):
        self.trainset = DFCDataset(
            "/netscratch/lscheibenreif/grss-dfc-20/",
            mode="test",
            transforms=True,
            clip_sample_values=True,
            used_data_fraction=1.0,
