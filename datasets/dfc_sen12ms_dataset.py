...
class DFCSEN12MSDataset:
    ...
    def get_patch_ids(self, season, scene_id, sensor=Sensor.s1):
        ...
        patch_ids = [
            ...
            for p in glob(os.path.join(path, "*.tif"))
        ]
    ...
    def get_s1_s2_lc_dfc_quad(
        ...
        s1, bounds1 = self.get_patch(
            season, scene_id, patch_id, s1_bands, window=window
        )
        s2, bounds2 = self.get_patch(
            season, scene_id, patch_id, s2_bands, window=window
        )
        lc, bounds3 = self.get_patch(
            season, scene_id, patch_id, lc_bands, window=window
        )

        if include_dfc:
            dfc, bounds4 = self.get_patch(
                season, scene_id, patch_id, dfc_bands, window=window
            )
...
if __name__ == "__main__":
    ...
    print(
        "Spring: {} scenes with a total of {} patches".format(
    ...
    s2_rgb_patch, bounds = sen12ms.get_patch(
        Seasons.SPRING, SCENE_ID, spring_ids[SCENE_ID][0], bands=S2Bands.RGB
    )
    ...
    for i, (scene_id, patch_ids) in enumerate(validation_ids.items()):
        ...
        s1, s2, lc, dfc, bounds = sen12ms.get_s1_s2_lc_dfc_quad(
            Seasons.TESTSET,
            scene_id,
            patch_ids[0],
            s1_bands=S1Bands.ALL,
        ...
        print(
            f"Scene: {scene_id}, S1: {s1.shape}, S2: {s2.shape}, LC: {lc.shape}, DFC: {dfc.shape}, Bounds: {bounds}"
        )

    print("\n")

    # Load all bands of all patches in a specified scene (scene 106)
    s1, s2, lc, dfc, _ = sen12ms.get_quad_stack(
        Seasons.SPRING,
        106,
        s1_bands=S1Bands.ALL,
        s2_bands=S2Bands.ALL,
